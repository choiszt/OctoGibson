...
def sim_process(task_name, scene_name, save_path, EVLM_name):
    ...
    while True:
        ...
        while True:
            if os.path.exists(
                    os.path.join(save_path, f'subtask_{subtask_iter}')):
                break
        sub_save_path = os.path.join(save_path, f'subtask_{subtask_iter}')
        with open('./EVLM_Task/new913.json', 'r') as f:
            a = json.load(f)
        ...
        init_pipeline(env,
                      robot,
                      camera,
                      task_name=str(gpt_task_name),
                      file_name=sub_save_path,
        ...
        while True:
            ...
            if isinstance(answer, str):
                ...
                with open(
                        f'./prompt_files/data/{gpt_name}/subtask_{subtask_iter}/action.py',
                        'w') as f:
                    f.write(heading)
                    f.write(code)
                # time.sleep(2)
                ...
                if (subtask_iter != 1):
                    sys.path.remove(
                        f'./prompt_files/data/{gpt_name}/subtask_{subtask_iter-1}'
                    )
                sys.path.append(
                    f'./prompt_files/data/{gpt_name}/subtask_{subtask_iter}')
                import action
                time.sleep(1)
                try:
                    reload(action)
        ...
        if critic == 'succeed':
            ...
            if signal:
                ...
                eu.save_feedback(feedback_path, subtask, code, error, critic,
                                 reset, main_succeed)
                break
            else:
                eu.save_feedback(feedback_path, subtask, code, error, critic,
                ...
                for iter_num in range(1, subtask_iter):
                    ...
                    if tmp_feedback['critic'] == 'succeed':
                        ...
                        module = importlib.import_module(
                            f'prompt_files.data.{gpt_name}.subtask_{iter_num}.action'
                        )
                        print(
                            f'prompt_files.data.{gpt_name}.subtask_{iter_num}.action retrieve'
                        )
...
sim_process(task_name=task_name,
            scene_name=scene,
            save_path=f'./prompt_files/data/{gpt_name}',
            EVLM_name=EVLM_name)
