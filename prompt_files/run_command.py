...
def run_command_in_terminal(command_args, process_name, conda_env):
    ...
    if process_name == 'gpt':
        ...
        command = f"python prompt_files/execute_sim.py -t {command_args['t']} -g {command_args['g']} -s {command_args['s']} -a {command_args['a']} -save {command_args['save']} -r {command_args['r']} -target {command_args['target']} --/log/level=error --/log/fileLogLevel=error --/log/outputStreamLevel=error >>./choiszt_log.txt"
